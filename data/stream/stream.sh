...
tmux send-keys -t $SESSION:$ElecID 'python electricity.py '"$strmDur"' '"$addr"'' C-m
echo "Electricity stream started."

# Create a new window for water data
WtrID=2
tmux new-window -t $WtrID
tmux send-keys -t $SESSION:$WtrID 'python water.py '"$strmDur"' '"$addr"'' C-m
echo "Water stream started."

# Create a new window for gas data
GsID=3
tmux new-window -t $GsID
tmux send-keys -t $SESSION:$GsID 'python gas.py '"$strmDur"' '"$addr"'' C-m
echo "Water stream started."
